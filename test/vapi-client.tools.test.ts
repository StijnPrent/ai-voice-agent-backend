...
describe('VapiClient tool dispatcher', () => {
  ...
  const createCompany = (hasGoogleIntegration = true) => {
    const company = new CompanyModel(1n, 'Test Company', 'info@test.com', '+31123456789', new Date(), new Date(), 'assistant-1', true, false, true, new Date());
  ...
  const execute = async (
    ...
    return (client as any).executeToolCall(call, session, callbacks);
  };
  ...
  it('returns an error when transfer handler is missing', async () => {
    const result = await execute(
      { id: 'tool-2', name: 'transfer_call', args: { phoneNumber: '+3100000000' } },
      baseCallbacks,
    );
  ...
  it('normalizes camelCase tool names for calendar availability', async () => {
    ...
    const result = await execute({
      id: 'tool-4b',
      name: 'checkGoogleCalendarAvailability',
      args: { date: '2024-01-09' },
    });
  ...
  it('rejects Google tools when integration is disabled', async () => {
    ...
    const result = await (client as any).executeToolCall(
      {
        id: 'tool-6',
        name: 'schedule_google_calendar_event',
    ...
    expect(result).toEqual({ success: false, error: 'Google integration not available' });
  });
});
